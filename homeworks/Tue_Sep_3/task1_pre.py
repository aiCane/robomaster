...
def move_chassis(_pid_speed: IncrementalPID, _ep_chassis: Robot.chassis) -> None:
    global distance, distances
    if distances:
        distance = distances[0]
        x_speed = _pid_speed.update(distance)
...
def move_robot(
    ...
    while True:
        ...
        if markers:
            ...
            y = marker[1]
            _ep_gimbal.drive_speed(
                pitch_speed=-_pid_pitch.update(y),
                yaw_speed=_pid_yaw.update(x)
            )
...
pid_speed = IncrementalPID(set_point=500, kp=0.0035, ki=0.000000005, kd=0.0001, max_output=0.5)
pid_pitch = IncrementalPID(set_point=0.5, kp=25, ki=0, kd=0.1)
pid_yaw = IncrementalPID(set_point=0.5, kp=50, ki=0, kd=0.1)
