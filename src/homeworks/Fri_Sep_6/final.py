...
def go(precision=3, max_count=10):
    ...
    if status == 0:
        ...
        ep_robot.vision.sub_detect_info(
            name='line', color='green', callback=on_detect_line
        )
        y = 0.0
...
def find_marker():
    ...
    if status == 0:
        ...
        sleep(
            0.05
        )  # change if (TypeError: 'int' object is not subscriptable) or (Index out of range)
        if is_aim_marker():
            print("marker!")
            status = 3
            deal_marker()
    ...
    while True:
        ...
        if lines:
            ...
            ep_robot.gimbal.moveto(
                pitch=-15, yaw=0, pitch_speed=90, yaw_speed=60
            ).wait_for_completed()
...
def init_robot():
    ...
    ep_robot.set_robot_mode(mode='chassis_lead')
    ep_robot.gimbal.moveto(
        pitch=-15, yaw=0, pitch_speed=90, yaw_speed=90
    ).wait_for_completed()
